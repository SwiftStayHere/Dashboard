...
export default function CopyLink({ hotelId, bookingId }) {
  let url = "";
  const [link, setLink] = useState((prev) => {
    if (url) {
      return `url/${hotelId}/${bookingId}`;
    } else {
      return `https://infoischia2.vercel.app/${hotelId}/${bookingId}`;
    }
  });
  useEffect(() => {
    setLink((prev) => {
